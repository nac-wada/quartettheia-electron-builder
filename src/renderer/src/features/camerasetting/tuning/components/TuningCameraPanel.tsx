...
import { CameraSettingsButtonGroup } from "./CameraSettingsButtonGroup";
import { CameraParameterForm } from "./CameraParameterForm";
import { CameraSettingPanelProps } from "../types";

export const TuningCameraPanel: FC<CameraSettingPanelProps> = ({ startIndex, deviceProps, videoId, fullScreen, setModal }) => {
  const { appTheme } = useAppTheme()
  const { id, nickname, ipv4Addr, macAddr, transport, networkInterface, primary } = deviceProps;
  ...
  return (
    ...
    >
      ...
      <CameraSettingsButtonGroup 
        device={{ ipv4Addr, primary, networkInterface, transport }}
        index={startIndex}
        setModal={setModal}
      />
      ...
      <CameraParameterForm ipv4Addr={ipv4Addr} transport={transport}/>
    </Card>
  )
}